...
var translations = map[string]map[string]string{
	...
	"show.fill_aired_help":  {"es": "Marca los episodios que faltan como vistos, con la fecha derivada de los episodios ya vistos de esa temporada (o la fecha de emisión si no hay ninguno visto). Corrige series que aparecen sin ver por diferencias de numeración. Si faltan los detalles, se obtienen de TMDB. No modifica los ya vistos; crea copia de seguridad antes.", "en": "Marks missing episodes as watched, dated from the episodes already watched in that season (or the air date if none are). Fixes shows that appear unwatched due to numbering mismatches. Missing details are fetched from TMDB. Won't touch episodes already watched; makes a backup first."},
	...
	"settings.tmdb_region":      {"es": "Región de plataformas (código país)", "en": "Providers region (country code)"},
	"settings.tmdb_region_help": {"es": "Código ISO de 2 letras (p.ej. ES, US) para mostrar en qué plataformas está disponible cada título.", "en": "Two-letter ISO country code (e.g. ES, US) used to show where each title streams."},
